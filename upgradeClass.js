...
class upgrade{
    ...
    onbuy(){
        if(this.inid == 62) {
            glitchstrikes[27].trigger();
            ups.forEach(x => x.ugtext());
        }
    }
    ...
    ugtext(){
        ...
        const eff = this.effectordefault(1);
        text +=  "<br>" + this.decription + "<br> <div class='lower'>";
        if(this.type != "unlock") {
            text += (this.type == "reset" ? `<br>reset for: ${eff}` :`<br> effect: ${eff}`);
        }
        this.element.innerHTML = text + "</div>";
    }
    ...
    tick(){
        this.buyable();
        if(player.tablefton != 0) return;
        if(typeof this.cost == "function") this.ugtext(); 
        else if(this.type != "unlock") this.element.lastElementChild.innerHTML = `${ this.type == "reset" ? "reset for" : "effect:"} ${this.effectordefault(1)}`
    }
